      ...
      <div class="table-responsive mt-2">
       ...
       <?php } elseif (isset($_GET['rid'])) { ?>
        <table class="table table-bordered" id="dynamic_field_chickin">
          ...
          <?php
          if($_GET['rid'] !=""){
            $product = "SELECT * FROM tbchick1 WHERE NOT chickin1=' ' and NOT chickin2=' ' and NOT chickin5=' ' and tb1_id='".$_GET['rid']."'";
            ...
            if ($result_product->num_rows > 0) {
              ...
              while($row_product = $result_product->fetch_assoc())
              {
                ...
                <td style="width:30%;">
                  <input type="text" name="chickin3[]" placeholder="Title" class="form-control" value="<?php echo utf8_encode(ucwords(strtolower($row_product['chickin3'])));?>" readonly/>
                </td>
                <td>
              ...
              if($_SESSION['branch'] == All) {
                $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickin1 from tbchick1) and not amount='0' and branch='$rf2' and purdate = CURDATE() and stat='Approved' and entry='IN' order by purdate desc;");
              }elseif($_SESSION['branch'] == Daet) {
                $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickin1 from tbchick1) and not amount='0' and branch='Daet' and purdate = CURDATE() and stat='Approved' and entry='IN' order by purdate desc;");
              }elseif($_SESSION['branch'] == Magsaysay) {
                $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickin1 from tbchick1) and not amount='0' and branch='Magsaysay' and purdate = CURDATE() and stat='Approved' and entry='IN' order by purdate desc;");
              }elseif($_SESSION['branch'] == Yashano) {
                $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickin1 from tbchick1) and not amount='0' and branch='Yashano' and purdate = CURDATE() and stat='Approved' and entry='IN' order by purdate desc;");
              }
              while($row=$res->fetch_array()){
                ?>
                <?php if($_SESSION['access'] == Administrator || $_SESSION['access'] == Manager || $_SESSION['access'] == BranchControl) { ?>
                  ...
                  <td style="width:30%;">
                    <input type="text" name="chickin3[]" placeholder="Title" class="form-control" value="<?php echo utf8_encode(ucwords(strtolower($row['title']))); ?>" readonly/>
                  </td>
                  <td>
  ...
  <div class="container">
    ...
    <?php } elseif (isset($_GET['rid'])) { ?>
      <table class="table table-bordered" id="dynamic_field_chickout">
        ...
        <?php
        if($_GET['rid'] !=""){
          $product = "SELECT * FROM tbchick2 WHERE NOT chickout1=' ' and NOT chickout2=' ' and NOT chickout5=' ' and tb1_id='".$_GET['rid']."'";
          $result_product = mysqli_query($conn, $product);
          ...
          if ($result_product->num_rows > 0) {
            ...
            <?php
            if($_SESSION['branch'] == All) {
              $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickout1 from tbchick2) and not amount='0' and branch='$rf2' and purdate = CURDATE() and stat='Approved' and entry='OUT' order by purdate desc;");
            }elseif($_SESSION['branch'] == Daet) {
              $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickout1 from tbchick2) and not amount='0' and branch='Daet' and purdate = CURDATE() and stat='Approved' and entry='OUT' order by purdate desc;");
            }elseif($_SESSION['branch'] == Magsaysay) {
              $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickout1 from tbchick2) and not amount='0' and branch='Magsaysay' and purdate = CURDATE() and stat='Approved' and entry='OUT' order by purdate desc;");
            }elseif($_SESSION['branch'] == Yashano) {
              $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickout1 from tbchick2) and not amount='0' and branch='Yashano' and purdate = CURDATE() and stat='Approved' and entry='OUT' order by purdate desc;");
            }
            while($row=$res->fetch_array()){
              ?>
              <?php if($_SESSION['access'] == Administrator || $_SESSION['access'] == Manager || $_SESSION['access'] == BranchControl) { ?>
                ...
                <td style="width:30%;">
                  <input type="text" name="chickout3[]" placeholder="Title" class="form-control" value="<?php echo utf8_encode(ucwords(strtolower($row['title']))); ?>" readonly/>
                </td>
                <td>
