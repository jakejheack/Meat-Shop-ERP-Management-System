...
<div id="collapseepcreditIn" class="collapse">
        ...
        <div class="table-responsive mt-2">
           <?php if(isset($_GET['view'])) { ?>
             <table class="table table-striped" id="dynamic_field_epcreditin" style="width: 100%;text-align:center;">
               ...
               <?php
               $res = $MySQLiconn->query("SELECT * FROM tbepcredit1 WHERE tb1_id=".$_GET['view']);
    ...
    <?php } elseif (isset($_GET['rid'])) { ?>
      <table class="table table-bordered" id="dynamic_field_epcreditin">
        ...
        <?php
        if($_GET['rid'] !=""){
            $product = "SELECT * FROM tbepcredit1 WHERE NOT epcreditin1=' ' and NOT epcreditin2=' ' and NOT epcreditin4=' ' and tb1_id='".$_GET['rid']."'";
            ...
            if ($result_product->num_rows > 0) {
                ...
                if($_SESSION['branch'] == All) {
                    $res = $MySQLiconn->query("select eid as id,transno as tno,created_on as co,empname as ename,amountin as paid from employees_credit where transno not in (select epcreditin4 from tbepcredit1) and not amountin='0' and branch='$rf2' and edate = CURDATE() and estat='Complete' order by edate desc;");
                }elseif($_SESSION['branch'] == Daet) {
                    $res = $MySQLiconn->query("select eid as id,transno as tno,created_on as co,empname as ename,amountin as paid from employees_credit where transno not in (select epcreditin4 from tbepcredit1) and not amountin='0' and branch='Daet' and edate = CURDATE() and estat='Complete' order by edate desc;");
                }elseif($_SESSION['branch'] == Magsaysay) {
                    $res = $MySQLiconn->query("select eid as id,transno as tno,created_on as co,empname as ename,amountin as paid from employees_credit where transno not in (select epcreditin4 from tbepcredit1) and not amountin='0' and branch='Magsaysay' and edate = CURDATE() and estat='Complete' order by edate desc;");
                }elseif($_SESSION['branch'] == Yashano) {
                    $res = $MySQLiconn->query("select eid as id,transno as tno,created_on as co,empname as ename,amountin as paid from employees_credit where transno not in (select epcreditin4 from tbepcredit1) and not amountin='0' and branch='Yashano' and edate = CURDATE() and estat='Complete' order by edate desc;");
                }
                while($row=$res->fetch_array()){                                       
                    ?>
                ...
                <tr style="text-align: center;">
                    ...
                    <tr>
                        ...
                        if($_SESSION['branch'] == Daet) {
                            $paidin = mysqli_num_rows( $MySQLiconn -> query("select eid as id,transno as tno,created_on as co,empname as ename,amountin as paid from employees_credit where transno not in (select epcreditin4 from tbepcredit1) and not amountin='0' and branch='Daet' and edate = CURDATE() and estat='Complete' order by edate desc;"));
                        }elseif($_SESSION['branch'] == Magsaysay) {
                            $paidin = mysqli_num_rows( $MySQLiconn -> query("select eid as id,transno as tno,created_on as co,empname as ename,amountin as paid from employees_credit where transno not in (select epcreditin4 from tbepcredit1) and not amountin='0' and branch='Magsaysay' and edate = CURDATE() and estat='Complete' order by edate desc;"));
                        }elseif($_SESSION['branch'] == Yashano) {
                            $paidin = mysqli_num_rows( $MySQLiconn -> query("select eid as id,transno as tno,created_on as co,empname as ename,amountin as paid from employees_credit where transno not in (select epcreditin4 from tbepcredit1) and not amountin='0' and branch='Yashano' and edate = CURDATE() and estat='Complete' order by edate desc;"));
                        }
...
<div id="collapseepcreditOut" class="collapse">
    <div class="container">
        <div class="table-responsive mt-2">
          ...
          <?php
          $res = $MySQLiconn->query("SELECT * FROM tbepcredit2 WHERE  tb1_id=".$_GET['view']);
          while($row=$res->fetch_array()){                                       
             ?>
             <tr>
...
<?php } elseif (isset($_GET['rid'])) { ?>
  <table class="table table-bordered" id="dynamic_field_epcreditout">
    ...
    <?php
    if($_GET['rid'] !=""){
        $product = "SELECT * FROM tbepcredit2 WHERE NOT epcreditout1=' ' and NOT epcreditout2=' ' and NOT epcreditout4=' ' and tb1_id='".$_GET['rid']."'";
        $result_product = mysqli_query($conn, $product);
        if ($result_product->num_rows > 0) {
            $xy=1;
            while($row_product = $result_product->fetch_assoc())
        ...
        <?php
        if($_SESSION['branch'] == All) {
            $res = $MySQLiconn->query("select eid as id,transno as tno,created_on as co,empname as ename,amountout as credit from employees_credit where transno not in (select epcreditout4 from tbepcredit2) and not amountout='0' and branch='$rf2' and edate = CURDATE() and estat='Complete' order by edate desc;");
        }elseif($_SESSION['branch'] == Daet) {
            $res = $MySQLiconn->query("select eid as id,transno as tno,created_on as co,empname as ename,amountout as credit from employees_credit where transno not in (select epcreditout4 from tbepcredit2) and not amountout='0' and branch='Daet' and edate = CURDATE() and estat='Complete' order by edate desc;");
        }elseif($_SESSION['branch'] == Magsaysay) {
            $res = $MySQLiconn->query("select eid as id,transno as tno,created_on as co,empname as ename,amountout as credit from employees_credit where transno not in (select epcreditout4 from tbepcredit2) and not amountout='0' and branch='Magsaysay' and edate = CURDATE() and estat='Complete' order by edate desc;");
        }elseif($_SESSION['branch'] == Yashano) {
            $res = $MySQLiconn->query("select eid as id,transno as tno,created_on as co,empname as ename,amountout as credit from employees_credit where transno not in (select epcreditout4 from tbepcredit2) and not amountout='0' and branch='Yashano' and edate = CURDATE() and estat='Complete' order by edate desc;");
        }
        while($row=$res->fetch_array()){
            ?>
            <?php if($_SESSION['access'] == Administrator || $_SESSION['access'] == Manager || $_SESSION['access'] == BranchControl) { ?>
    ...
    <table class="table table-bordered" id="dynamic_field_epcreditout">
        ...
        <tr style="text-align: center;">
            ...
            <tr>
                <?php
                if($_SESSION['branch'] == Daet) {
                    $creditout = mysqli_num_rows( $MySQLiconn -> query("select eid as id,transno as tno,created_on as co,empname as ename,amountout as credit from employees_credit where transno not in (select epcreditout4 from tbepcredit2) and not amountout='0' and branch='Daet' and edate = CURDATE() and estat='Complete' order by edate desc;"));
                }elseif($_SESSION['branch'] == Magsaysay) {
                    $creditout = mysqli_num_rows( $MySQLiconn -> query("select eid as id,transno as tno,created_on as co,empname as ename,amountout as credit from employees_credit where transno not in (select epcreditout4 from tbepcredit2) and not amountout='0' and branch='Magsaysay' and edate = CURDATE() and estat='Complete' order by edate desc;"));
                }elseif($_SESSION['branch'] == Yashano) {
                    $creditout = mysqli_num_rows( $MySQLiconn -> query("select eid as id,transno as tno,created_on as co,empname as ename,amountout as credit from employees_credit where transno not in (select epcreditout4 from tbepcredit2) and not amountout='0' and branch='Yashano' and edate = CURDATE() and estat='Complete' order by edate desc;"));
                }
