      ...
      <div class="table-responsive mt-2">
       ...
       <?php } elseif (isset($_GET['rid'])) { ?>
        <table class="table table-bordered" id="dynamic_field_chickin">
          ...
          <?php
          if($_GET['rid'] !=""){
            $product = "SELECT * FROM tbchick1_v3 WHERE NOT chickin1=' ' and NOT chickin2=' ' and NOT chickin5=' ' and tb1_id='".$_GET['rid']."'";
            ...
            if ($result_product->num_rows > 0) {
              ...
              if($_SESSION['branch'] == All) {
                ...
                $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickin1 from tbchick1_v3) and not amount='0' and branch='Magsaysay' and purdate = CURDATE() and stat='Approved' and entry='IN' order by purdate desc;");
              }elseif($_SESSION['branch'] == Yashano) {
                $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickin1 from tbchick1_v3) and not amount='0' and branch='Yashano' and purdate = CURDATE() and stat='Approved' and entry='IN' order by purdate desc;");
              }
              while($row=$res->fetch_array()){
                ?>
  ...
  <div class="container">
    ...
    <?php } elseif (isset($_GET['rid'])) { ?>
      <table class="table table-bordered" id="dynamic_field_chickout">
        ...
        <?php
        if($_GET['rid'] !=""){
          $product = "SELECT * FROM tbchick2_v3 WHERE NOT chickout1=' ' and NOT chickout2=' ' and NOT chickout5=' ' and tb1_id='".$_GET['rid']."'";
          $result_product = mysqli_query($conn, $product);
          ...
          if ($result_product->num_rows > 0) {
            ...
            <?php
            if($_SESSION['branch'] == All) {
              $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickout1 from tbchick2_v3) and not amount='0' and branch='$rf2' and purdate = CURDATE() and stat='Approved' and entry='OUT' order by purdate desc;");
            }elseif($_SESSION['branch'] == Daet) {
              ...
              $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickout1 from tbchick2_v3) and not amount='0' and branch='Magsaysay' and purdate = CURDATE() and stat='Approved' and entry='OUT' order by purdate desc;");
            }elseif($_SESSION['branch'] == Yashano) {
              $res = $MySQLiconn->query("select created_on as pdate,purchaseno as pno,supplierid as sid,supplier as sname,title as title,amount as amount from purchase where purchaseno not in (select chickout1 from tbchick2_v3) and not amount='0' and branch='Yashano' and purdate = CURDATE() and stat='Approved' and entry='OUT' order by purdate desc;");
            }
            while($row=$res->fetch_array()){
              ?>
              <?php if($_SESSION['access'] == Administrator || $_SESSION['access'] == Manager || $_SESSION['access'] == BranchControl) { ?>
